...
if (isset($_SESSION["user"])) {
    ...
    if ($_SESSION["user"]["user_type_id"] == "1" || $_SESSION["user"]["user_type_id"] == "2" || $_SESSION["user"]["user_type_id"] == "3" || $_SESSION["user"]["user_type_id"] == "4") {
        ...
        // $guarantorIncomeBusiness = $_POST["guarantorIncomeBusiness"];
        ...
        if (empty($guarantorEmpName)) {
            ...
            if ($guarantor_num == 0) {
                ...
                if (isset($_FILES["guarantorNicFrontPhoto"])) {
                    Database::iud("UPDATE `guarantor` SET `nic_font_img_path`=? WHERE `id`=?;", [$guarantorNicFrontPhotoPath, $guarantor_id], "ss");
                }

                if (isset($_FILES["guarantorNicBackPhoto"])) {
                    Database::iud("UPDATE `guarantor` SET `nic_back_img_path`=? WHERE `id`=?;", [$guarantorNicBackPhotoPath, $guarantor_id], "ss");
                }

                Database::iud("UPDATE `guarantor` SET `full_name`=?,`name_with_initial`=?,`address`=?,`dob`=?,`nic`=?,`tel`=?,`mobile`=?,`user_title_id`=?,`spouse_full_name`=?,`spouse_nic`=?,`spouse_tel`=?,`spouse_profession`=?,`business_name`=?,`business_address`=?,`business_reg_no`=?,`business_nature`=?,`employment_income`=?,`other_income`=?,`living_cost`=?,`loan_repayment`=?,`other_details`=? WHERE `insurance_file_id`=? ;", [$guarantorFullName, $guarantorNameWithInitial, $guarantorAddress, $guarantorDOB, $guarantorNIC, $guarantorTelNo, $guarantorMobile, $guarantorTitle, $guarantorSpouseFullName, $guarantorSpouseNIC, $guarantorSpouseTel, $guarantorSpouseProfession, $guarantorEmpName, $guarantorEmpAddress, $guarantorBusinessRegNo, $guarantorBusinessNature, $guarantorIncomeEmp, $guarantorIncomeOther, $guarantorCostLiving, $guarantorLoanRepayment, $guarantorOtherDetails, $insurance_file_id], "ssssssssssssssssssssss");
